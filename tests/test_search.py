...
def mock_embed(mocker: MockerFixture):
    return mocker.patch("src.services.search.embed_text", return_value=[0.1, 0.2, 0.3])


@pytest.fixture()
def mock_query(mocker: MockerFixture):
    return mocker.patch(
        "src.services.search.query_vectors",
        return_value=[
            {
                "id": "job-1",
                "score": 0.95,
...
@pytest.fixture()
def mock_download_url(mocker: MockerFixture):
    return mocker.patch(
...
def test_search_returns_ranked_results(
    mock_embed, mock_query, mock_download_url
) -> None:
    """Results preserve Pinecone order (highest score first) and each has a presigned URL."""
...
def test_search_empty_query_returns_empty(mocker: MockerFixture) -> None:
    """Empty query returns immediately with no results and no Pinecone call."""
...
def test_search_pinecone_error_returns_error_field(mocker: MockerFixture) -> None:
    """Exception during embedding or querying returns SearchResponse with error field set."""
