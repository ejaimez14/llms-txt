...
def _mock_sdk(mocker: MockerFixture, exc: Exception | None = None) -> None:
    ...
    mocker.patch("src.tasks.ui_planner._run_sdk", side_effect=_sdk)
...
def test_claude_success_path_calls_hooks(mocker: MockerFixture) -> None:
    mock_hooks = mocker.patch("src.tasks.ui_planner.JobHooks", return_value=MagicMock())
    _mock_sdk(mocker)
...
def test_claude_error_path_calls_on_error_and_does_not_reraise(
    ...
    run_ui_planner_task("job-1", "https://example.com", "claude")  # must not raise
